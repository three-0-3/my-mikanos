...
extern "C" void main(int argc, char** argv) {
  ...
  for (;;) {
    ...
    // add up z of the center of the surface (multiplied by 4)
...
void DrawObj(uint64_t layer_id) {
  // convert object coordinates vert to screen coordinates src (z+ at the back)
  for (int i = 0; i < kCube.size(); i++) {
  ...
  for (;;) {
    // draw from the object at the back (i.e. larger z) to the front
    ...
    // draw only if the normal vector faces screen
...
void DrawSurface(uint64_t layer_id, int sur) {
  ...
  int ymin = kCanvasSize, ymax = 0; // scope of drawing [ymin, ymax]
  int y2x_up[kCanvasSize], y2x_down[kCanvasSize]; // set of Y, X
  for (int i = 0; i < surface.size(); i++) {
    ...
    int* y2x;
    int x0, y0, y1, dx;
    if (p0.y < p1.y) { // p0 --> p1 goes up
      y2x = y2x_up;
      x0 = p0.x; y0 = p0.y; y1 = p1.y; dx = p1.x - p0.x;
    } else { // p0 --> p1 goes down
      y2x = y2x_down;
